...
Sistem pembayaran rumah sakit berbasis web menggunakan Laravel 11 dengan fitur manajemen voucher diskon, transaksi pembayaran, dashboard marketing, dan laporan otomatis via email & Telegram.
...
- **Kasir**: Menangani transaksi pembayaran pasien, pemilihan layanan medis, dan penerapan voucher atau diskon asuransi
- **Marketing**: Mengelola voucher, analisis statistik penggunaan voucher, dan monitoring dashboard
...
│  │   Daily Report @ 23:45 WIB → Email + Telegram         │  │
│  └────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────┘
```
...
2. Input data pasien (nama, NIK)
3. Pilih layanan medis yang digunakan
4. Pilih salah satu metode diskon:
   - **Voucher**: Cek kode voucher → sistem validasi → diskon diterapkan
   - **Insurance**: Input NIK + nomor kartu → API validation → diskon diterapkan
5. Sistem menghitung total pembayaran setelah diskon
6. Simpan transaksi → generate kode transaksi otomatis
7. Cetak struk PDF

**Catatan**: Voucher dan Insurance **tidak dapat digunakan bersamaan** dalam satu transaksi.

### 📊 Flow Laporan Harian

1. **Task Scheduler** berjalan otomatis setiap hari pukul **23:45 WIB**
2. Sistem mengumpulkan semua transaksi hari ini
3. Generate laporan Excel dengan detail transaksi
4. Kirim laporan ke:
   - **Email** (dengan attachment Excel)
   - **Telegram** (dengan format teks informatif)

## ✨ Fitur Utama

### 1. **Role Management**
- **Kasir**: Mengelola transaksi pembayaran pasien, input data, apply voucher/asuransi
...
- Chart pemakaian voucher per bulan
...
- ✅ **API Integration** - Integrasi dengan RS Delta Surya untuk validasi
...
**Catatan Penting**: Voucher dan Insurance Discount **tidak dapat digunakan bersamaan** dalam satu transaksi.
...
### 7. **Cron Job Laporan Harian** 🆕
- Otomatis kirim laporan transaksi harian ke email dan Telegram
- Excel attachment dengan detail transaksi
- Scheduled otomatis setiap hari pukul **23:45 WIB**
...
- Dokumentasi lengkap: [API_ENDPOINTS.md](API_ENDPOINTS.md)
